...
clean() {
    find "${OPENLDAP_BACKUP_DIR}" -type f -mtime "+$1" -print0 | xargs -0 rm -fv | container logger info
}
...
if [ "${CONFIG}" = true ]; then
    container logger info "Backing up OpenLDAP config ..."
    backup_database "${FILE_PREFIX}${OPENLDAP_CTL_BACKUP_CONFIG_FILE_SUFFIX}" 0
fi

if [ "${DATA}" = true ]; then
    container logger info "Backing up OpenLDAP data ..."
    backup_database "${FILE_PREFIX}${OPENLDAP_CTL_BACKUP_DATA_FILE_SUFFIX}" 1
fi

if [ -n "${CLEAN_DAYS}" ]; then
    container logger info "Cleaning up backups older than ${CLEAN_DAYS} day(s) ..."
    clean "${CLEAN_DAYS}"
fi
