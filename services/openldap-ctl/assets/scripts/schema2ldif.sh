...
for schema in ${DEPENDENCIES}; do
  ...
  if [ ! -e "${schema_path}" ]; then
    container logger fatal "File not found: ${schema_path}"
  fi
...
if ! /usr/sbin/slaptest -f "${TMP_FILE}" -F "${TMP_DIR}" > /dev/null ; then
    container logger fatal "Failed do convert ${SCHEMAS} to ldif"
fi
...
for schema in ${SCHEMAS} ; do
    ...
    if [ -e "${ldif}" ]; then
      container logger fatal "${schema_name} ldif file already exists: ${ldif}"
    fi

    find "${TMP_DIR}" | container logger debug
