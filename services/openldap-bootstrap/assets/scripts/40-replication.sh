...
if [ "${OPENLDAP_BOOTSTRAP_REPLICATION}" = "true" ]; then
    ...
    if [ "${TYPE}" = "env" ]; then
        ...
        if [ "${i}" -ne 3 ]; then
            container logger fatal "OPENLDAP_BOOTSTRAP_REPLICATION_HOSTS must contain exactly 2 hosts, got $((i - 1)) (${OPENLDAP_BOOTSTRAP_REPLICATION_HOSTS})"
        fi
        ...
        if [ -z "${OPENLDAP_BOOTSTRAP_REPLICATION_CONFIG_READONLY_PASSWORD}" ]; then
            ...
            container logger warning "Generated config replicator user password: ${OPENLDAP_BOOTSTRAP_REPLICATION_CONFIG_READONLY_PASSWORD}"
        fi
        ...
        if [ -z "${OPENLDAP_BOOTSTRAP_REPLICATION_DATA_READONLY_PASSWORD}" ]; then
            ...
            container logger warning "Generated data replicator user password: ${OPENLDAP_BOOTSTRAP_REPLICATION_DATA_READONLY_PASSWORD}"
        fi
        ...
        OPENLDAP_BOOTSTRAP_REPLICATION_DATA_SYNC_REPL=$(container envsubst --quiet "${OPENLDAP_BOOTSTRAP_REPLICATION_DATA_SYNC_REPL_TEMPLATE}")
