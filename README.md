...
```bash
docker exec openldap container entrypoint -x openldap-ctl -- backup my-backup
```

Create only a data backup and remove files older than 15 days:

```bash
docker exec openldap container entrypoint -x openldap-ctl -- backup my-backup --data --clean 15
```
...
``` bash
docker exec openldap container entrypoint -x openldap-ctl -- restore my-backup --force
```
...
docker run
  -v schema:/data
  --rm osixia/openldap --quiet -x openldap-ctl -- schema2ldif /data/test.schema
```
...
``` bash
docker run --rm osixia/openldap --quiet -x openldap-ctl -- password generate
```

Hash a password:

``` bash
docker run --rm osixia/openldap --quiet -x openldap-ctl -- password hash passw0rd
```
...
| `OPENLDAP_CRON_JOB`                      | Cron job used by the backup service                         | `15 2 * * * PATH=${PATH} source /run/container/environment.sh && container entrypoint -x openldap-ctl -- backup "$(date -I)-${OPENLDAP_VERSION}-openldap-cron" --clean 15` |
